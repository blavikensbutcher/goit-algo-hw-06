...
dijkstra = single_source_dijkstra_path(G, source="Дніпро", weight="weight")

dijkstra_realization = dijkstra_r(G, "Дніпро")
...
print(
    f"\n{BOLD}{CYAN}=== Найкоротші шляхи за алгоритмом Дейкстри (від Дніпра) ==={RESET}"
)
for target_city, path in dijkstra.items():
    if target_city:
        print(f"{BOLD}{target_city}{RESET}: {path}")
        print(
            f"\n{BOLD}{CYAN}=== Найкоротші шляхи за алгоритмом Дейкстри (від Дніпра) Реалізація ==={RESET}"
        )
